...
namespace Plays.tv.Database
{
    public class VideoSQLiteContext:IVideoContext
    {
        ...
        public Video GetVideo(int videoid)
        {
            ...
            {
                //query 
                string query = "SELECT ACCOUNT.NAME, ACCOUNT.EMAIL, ACCOUNT.PASSWORD, ACCOUNT.NICKNAME, ACCOUNT.PROFILEPIC,  VIDEO.ID, VIDEO.TITLE, VIDEO.LINK, VIDEO.VIEWS, G.ID, G.NAME, C.NAME, C.VIEWS, ACCOUNT.ID, SUM(F.[LIKE]), SUM(F.DISLIKE) FROM ACCOUNT FULL OUTER JOIN VIDEO ON VIDEO.ACCOUNT_ID = ACCOUNT.ID JOIN GAME G ON G.ID = VIDEO.GAME_ID LEFT JOIN CATEGORY C ON C.ID = VIDEO.CATEGORY_ID LEFT JOIN FEEDBACK F ON F.VIDEO_ID = VIDEO.ID GROUP BY ACCOUNT.NAME, ACCOUNT.EMAIL, ACCOUNT.PASSWORD, ACCOUNT.NICKNAME, ACCOUNT.PROFILEPIC, VIDEO.ID, VIDEO.TITLE, VIDEO.LINK, VIDEO.VIEWS, G.ID, G.NAME, C.NAME, C.VIEWS, ACCOUNT.ID HAVING VIDEO.ID = @id ";
                //opent de connectie vanuit de database classe            
                SqlCommand cmd = new SqlCommand(query, conn);
                ...
                if (reader.HasRows)
                {
                    while (reader.Read() && !breakflag)
                    {
                        ...
                        int gameid = reader.GetInt32(9);
                        string gameTitle = reader.GetString(10);
                        string categoryName = reader.GetString(11);
                        int categoryViews = reader.GetInt32(12);
                        int likes = 0;
                        int dislikes = 0;
                        if (!reader.IsDBNull(14))
                        {
                            likes = reader.GetInt32(14);
                        }

                        if (!reader.IsDBNull(15))
                        {
                            dislikes = reader.GetInt32(15);
                        }

                        User author = new User(authorid, name, email, password, nickname, profilepic);
                        Game game = new Game(gameid, gameTitle);
                        Video newVideo = new Video(id, author, views, title, new Category(categoryName, categoryViews), new Feedback(likes, dislikes), game, link);
                        result = newVideo;
                    }
                    // Check wat role is returned, based on role it initializes a different account
