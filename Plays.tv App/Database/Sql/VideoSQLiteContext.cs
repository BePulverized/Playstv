...
namespace Plays.tv.Database
{
    public class VideoSQLiteContext:IVideoContext
    {
        ...
        public bool Insert(Video video)
        {
            using (SqlConnection connection = new SqlConnection(Database.Connection.ConnectionString))
            {
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    command.CommandText = "INSERT INTO VIDEO(ACCOUNT_ID, VIEWS, TITLE, GAME_ID, CATEGORY_ID, LINK, DATA, THUMBNAIL) VALUES(@account_id, @views, @title, @game_id, 1, @link, @data, @thumbnail)";
                    ...
                    command.Parameters.AddWithValue("@data", video.Data);
        ...
        // Get all the reactions for a certain video
        ...
        // Getting all videos for a certain user
            ...
            using (SqlConnection conn = Database.Connection)
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read() && !breakflag)
                    {
                        ...
                        Video newVideo = new Video(id, author, views, title, new Category(categoryName, categoryViews),
                            new Feedback(likes, dislikes), game);
                        result.Add(newVideo);
                    }
                    // Check wat role is returned, based on role it initializes a different account
                }
